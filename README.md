...
For test.c, you must include "std.h" at the top of the file. You cannot use any other standard libraries. The main function must begin with ENTRY (defined in std.h). Use puti(int) to print an integer. Use poparg(int) to get an argument passed to the batch file (1 through 9). Use puts(string) to print a string (must end with ;). See the provided test.c for an example.
