...
For test.c, you must include "std.h" at the top of the file. You cannot use any other standard libraries. The main function must begin with ENTRY (defined in std.h). Use puti(int) to print an integer. Use poparg(int) to get an argument passed to the batch file (1 through 9). Note that if the argument was not passed, undefined behavior may occur. poparg returns an integer, so all arguments must be integers. Use puts(string) to print a string. The string must end with a semicolon (;). You may only use characters a-z A-Z 0-9. See the provided test.c for an example program.
